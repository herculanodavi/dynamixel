...
int main() {
    ...
    dxl2_inst_packet_t inst_packet;
    inst_packet.id = 0x01;
    inst_packet.instruction = DXL2_INST_SYNC_READ;
    memcpy(inst_packet.params, params, params_length);
    inst_packet.params_length = params_length;

    int num_bytes = dxl2_serialize_instruction_packet(&inst_packet, buffer, buffer_length);
